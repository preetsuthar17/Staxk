...
function useNameEditing(initialName: string | null, workspaceSlug: string) {
...
function useSlugEditing(initialSlug: string | null, workspaceSlug: string) {
  ...
  useEffect(() => {
    ...
    slugDebounceRef.current = setTimeout(async () => {
      try {
        ...
        const data = await response.json();
        setSlugAvailability(
          response.ok ? (data?.available ?? false) : false
        );
      } catch {
        setSlugAvailability(null);
      } finally {
  ...
  const handleSaveSlug = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/workspace/${workspaceSlug}`, {
        ...
        body: JSON.stringify({ slug: trimmedSlug.toLowerCase() }),
      });
      ...
      window.location.href = `/${normalizedSlug}/settings`;
    } catch (error) {
      toast.error(getErrorMessage(error, "Failed to update slug"));
    } finally {
      setIsSavingSlug(false);
    }
  }, [slug, slugAvailability, workspaceSlug]);
...
function useLogoUpload(initialLogo: string | null, workspaceSlug: string) {
...
export function WorkspaceGeneralCard() {
  ...
  const {
    ...
    handleCancelEditSlug,
    handleSaveSlug,
  } = useSlugEditing(workspaceData?.slug || null, workspaceSlug);
  ...
  const {
    ...
    handleLogoClick,
    handleLogoChange,
  } = useLogoUpload(workspaceData?.logo || null, workspaceSlug);
